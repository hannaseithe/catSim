...
def test_cat_instantiation(sample_cat:Cat):
    ...
    assert sample_cat.is_at_home() == True
...
def test_cat_leave(sample_cat:Cat):
    sample_cat.leave(1)
    assert sample_cat.is_on_the_edge() == True
...
def test_cat_arrive(sample_cat:Cat):
    ...
    assert sample_cat.is_on_the_edge() == False
...
def test_relationship_instantiation(sample_rel:Relationship):
    ...
    assert sample_rel.metrics == None
...
def test_cat_metrics_instantiation():
    kwargs = {
        ...
        "amount_of_cats_interacted_with": 8,
        "amount_of_friends": 1,
        "amount_of_enemies": 2,
...
def test_relationship_method_is_relationship(sample_rel:Relationship):
    assert sample_rel.is_relationship(0,1) == True
    assert sample_rel.is_relationship(1,0) == True
    assert sample_rel.is_relationship(1,1) == False
    assert sample_rel.is_relationship(2,1) == False
...
def test_edge_method_node_in_edge(sample_edge):
    assert sample_edge.node_in_edge(0) == True
    assert sample_edge.node_in_edge(1) == True
    assert sample_edge.node_in_edge(2) == False
